...
template <typename T, typename COMP> class Heap{
	private:
		...
		void buildHeap(int num){
			//因为堆是完全二叉树，因此有一半的节点都是叶节点，没必要siftdown
			//而且这些叶节点都是位于数组的末端的
	...
	public:
		...
		Heap(T eles[], int num, bool M=true, int Max=20){
			if (num > Max){
		...
		void printHeap(){
			...
			std::cout << std::endl << "元素个数：" << Size << std::endl;
		}
};
